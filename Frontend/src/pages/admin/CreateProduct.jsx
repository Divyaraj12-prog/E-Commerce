                ...
                <div className="flex items-center gap-3 mb-8">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            <span className="px-2 py-0.5 bg-violet-100 text-violet-700 text-xs font-semibold rounded-full">Admin</span>
                            <h1 className="text-2xl font-bold text-zinc-900">Create New Product</h1>
                        </div>
                        <p className="text-zinc-500 text-sm">Add a new product to your store</p>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-3xl border border-zinc-100 shadow-sm p-8">
                    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
                        ...
                        <div className="flex gap-4 items-start flex-col sm:flex-row">
                            <div className="w-full sm:w-36 h-36 bg-zinc-50 border-2 border-dashed border-zinc-200 rounded-2xl flex items-center justify-center overflow-hidden shrink-0">
                                {imageUrl ? (
                                    <img src={imageUrl} alt="preview" className="w-full h-full object-contain p-2" onError={e => e.target.style.display = 'none'} />
                                ) : (
                            ...
                            <div className="flex-1 w-full">
                                ...
                                <p className="text-xs text-zinc-400 mt-2">Paste a direct image URL to see a live preview above.</p>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div>
                                <label className="block text-sm font-medium text-zinc-700 mb-1.5">Initial Rating <span className="text-zinc-400">(0–5)</span></label>
                                <input
                                    type="number"
                                    step="0.1"
                                    min="0"
                                    max="5"
                                    placeholder="e.g. 4.2"
                                    {...register("rating.rate", { min: { value: 0, message: "Min 0" }, max: { value: 5, message: "Max 5" } })}
